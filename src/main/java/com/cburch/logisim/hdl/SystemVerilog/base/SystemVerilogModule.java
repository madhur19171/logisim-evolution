...
public class SystemVerilogModule extends InstanceFactory implements HdlModelListener {

  static final Logger logger = LoggerFactory.getLogger(SystemVerilogModule.class);
  static final Attribute<String> nameAttr = Attributes.forString("systemVerilogModule", Strings.S.getter("systemVerilogModuleName"));
  static final ArithmeticIcon icon = new ArithmeticIcon("SYSTEMVERILOG"); // TODO: Make the name smaller
  ...
  private final SystemVerilogContent content;
  private final ArrayList<Instance> myInstances;

  public SystemVerilogModule(SystemVerilogContent content) {
    super("", null, new SystemVerilogHdlGeneratorFactory(), true);
    this.content = content;
    this.content.addHdlModelListener(this);
    this.setIcon(icon);
    icon.setInvalid(!content.isValid());
    setFacingAttribute(StdAttr.FACING);
    appearance = SystemVerilogAppearance.create(getPins(), getName(), StdAttr.APPEAR_EVOLUTION);
    myInstances = new ArrayList<>();
  }

  public void setSimName(AttributeSet attrs, String sName) {
    if (attrs == null) return;
    final var atrs = (SystemVerilogModuleAttributes) attrs;
    final var label = ("".equals(attrs.getValue(StdAttr.LABEL))) ? sName : getHDLTopName(attrs);
    if (atrs.containsAttribute(SystemVerilogConstants.SIM_NAME_ATTR))
      atrs.setValue(SystemVerilogConstants.SIM_NAME_ATTR, label);
  }

  public String getSimName(AttributeSet attrs) {
    if (attrs == null) return null;
    final var atrs = (SystemVerilogModuleAttributes) attrs;
    return atrs.getValue(SystemVerilogConstants.SIM_NAME_ATTR);
  }

  @Override
  public String getName() {
    if (content == null) return "System Verilog Module";
    else return content.getName();
  }

  @Override
  public StringGetter getDisplayGetter() {
    if (content == null) return Strings.S.getter("systemVerilogComponent");
  ...
  protected void configureNewInstance(Instance instance) {
    final var attrs = (SystemVerilogModuleAttributes) instance.getAttributeSet();
  ...
  public AttributeSet createAttributeSet() {
    return new SystemVerilogModuleAttributes(content);
  }
  ...
  public void paintInstance(InstancePainter painter) {
    final var attrs = (SystemVerilogModuleAttributes) painter.getAttributeSet();
  ...
  /**
   * Propagate signals through the SystemVerilog component. Logisim doesn't have a SystemVerilog simulation tool. So
   ...
   * Logisim. Then we can set the SystemVerilog component output properly.
   *
  ...
  public void propagate(InstanceState state) {
    ...
    if (state.getProject().getSystemVerilogSimulator().isEnabled()
        && state.getProject().getSystemVerilogSimulator().isRunning()) {

      final var systemVerilogSimulator = state.getProject().getSystemVerilogSimulator();
      ...
      for (final var singlePort : state.getInstance().getPorts()) {
        ...
        final var systemVerilogModuleName = getSimName(state.getAttributeSet());

        String message =
            singlePort.getType()
                + ":"
                + systemVerilogModuleName
                + "_"
        ...
        systemVerilogSimulator.send(message);
      }

      systemVerilogSimulator.send("sync");

      /* Get response from tcl server */
      String serverResponse;
      while ((serverResponse = systemVerilogSimulator.receive()) != null
      ...
      /* SystemVerilogSimulation stopped/disabled */
    } else {
      ...
      throw new UnsupportedOperationException(
          "SystemVerilog component simulation is not supported. This could be because there is no Questasim/Modelsim simulation server running.");     // FIXME: hardcoded string
    }
  }

  /**
   * Save the SystemVerilog entity in a file. The file is used for SystemVerilog components simulation by
   * QUestasim/Modelsim
   */
  public void saveFile(AttributeSet attrs) {
    ...
    try {
      writer =
          new PrintWriter(SystemVerilogConstants.SIM_SRC_PATH + getSimName(attrs) + ".sv",
      ...
      logger.error("Could not create SystemVerilog file: {}", e.getMessage());     // FIXME: hardcoded string
      e.printStackTrace();
    }
  }

  private SystemVerilogAppearance appearance;
  ...
  void updatePorts(Instance instance) {
    ...
    appearance = SystemVerilogAppearance.create(getPins(), getName(), style);
